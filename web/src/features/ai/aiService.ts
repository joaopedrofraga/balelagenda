...
export async function suggestPlaces(input: SuggestPlacesInput): Promise<SuggestPlacesResult> {
  ...
  const { data, error } = await supabase.functions.invoke<SuggestPlacesResult | { error: string }>(
    'suggest-places',
    {
      body: {
  ...
  if (!error && data && typeof data === 'object' && 'filters' in data && 'places' in data) {
    return data as SuggestPlacesResult
  }

  // Fallback: interpreta localmente; não inventa estabelecimentos
  const filters =
